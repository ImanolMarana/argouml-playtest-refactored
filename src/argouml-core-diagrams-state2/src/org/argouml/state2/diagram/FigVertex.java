...
public class FigVertex extends FigNodeModelElement {
    ...
    public void setEnclosingFig(Fig encloser) {
        ...
        Object region = findRegion(encloser, lp);
    ...
    private Object findRegion(Fig encloser, LayerPerspective lp) {
        Object region = null;
        if (encloser != null) {
            region = getRegionFromEncloser(encloser);
        } else {
            // The vertex was dropped onto the diagram.
            region = getRegionFromDiagram(lp);
        }
        return region;
    }
    
    private Object getRegionFromEncloser(Fig encloser) {
        Object region;
        ...
        if (regions.isEmpty()) {
            ...
            region = findRegionContainingBounds(regions, compositeState);
        }
        return region;
    }

    private Object findRegionContainingBounds(List regions,
            FigVertex compositeState) {
        Object region = null;
        for (DiagramElement de : compositeState.regionCompartment
                .getDiagramElements()) {
    ...
    private Object getRegionFromDiagram(LayerPerspective lp) {
        Object region;

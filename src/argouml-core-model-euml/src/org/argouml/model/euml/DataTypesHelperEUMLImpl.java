...
class DataTypesHelperEUMLImpl implements DataTypesHelper {
    ...
    public void modifyValueSpecification(Object handle, Object[] tabValues) {
        if (handle instanceof LiteralSpecification) {
            modifyLiteralSpecification((LiteralSpecification) handle, tabValues);
        } else if (handle instanceof OpaqueExpression) {
            modifyOpaqueExpression((OpaqueExpression) handle, tabValues);
        }
        // TODO Expression and InstanceValue (how)
        // If anyone have an example.
    }
    
    private void modifyLiteralSpecification(LiteralSpecification ls, Object[] tabValues) {
        if (ls instanceof LiteralBoolean) {
            modifyLiteralBoolean((LiteralBoolean) ls, tabValues);
        } else if (ls instanceof LiteralString) {
            modifyLiteralString((LiteralString) ls, tabValues);
        } else if (ls instanceof LiteralUnlimitedNatural) {
            modifyLiteralUnlimitedNatural((LiteralUnlimitedNatural) ls, tabValues);
        } else if (ls instanceof LiteralInteger) {
            modifyLiteralInteger((LiteralInteger) ls, tabValues);
        } 
        // LiteralNull can't be changed
    }
    
    private void modifyLiteralBoolean(LiteralBoolean lb, Object[] tabValues) {
        if (tabValues != null && tabValues.length == 1) {
            lb.setValue((Boolean) tabValues[0]);
        }
    }
    
    private void modifyLiteralString(LiteralString ls, Object[] tabValues) {
        if (tabValues != null && tabValues.length == 1) {
            ls.setValue((String) tabValues[0]);
        }
    }
    
    private void modifyLiteralUnlimitedNatural(LiteralUnlimitedNatural lun, Object[] tabValues) {
        if (tabValues != null && tabValues.length == 1) {
            lun.setValue((Integer) tabValues[0]);
        }
    }
    
    private void modifyLiteralInteger(LiteralInteger li, Object[] tabValues) {
        if (tabValues != null && tabValues.length == 1) {
            li.setValue((Integer) tabValues[0]);
        }
    }
    
    private void modifyOpaqueExpression(OpaqueExpression oe, Object[] tabValues) {

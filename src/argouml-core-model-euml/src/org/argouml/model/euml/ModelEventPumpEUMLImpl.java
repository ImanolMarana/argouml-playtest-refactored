...
class ModelEventPumpEUMLImpl extends AbstractModelEventPump {
    ...
    List<EventAndListeners> events = generateEvents(
        notifier, oldValue, newValue, eventType, featureName, oppositeRef);

    fireEventAndListeners(events);
  }
  
  private List<EventAndListeners> generateEvents(
      Object notifier,
      Object oldValue,
      Object newValue,
      int eventType,
    ...
    if (eventType == Notification.SET) {
      ...
      events.addAll(generateAddRemoveEvents(
          notifier, oldValue, newValue, eventType, featureName, oppositeRef));
    }
  ...
  private List<EventAndListeners> generateAddRemoveEvents(
      Object notifier,
      Object oldValue,
      Object newValue,
      int eventType,
    ...
    if (eventType == Notification.ADD) {
      ...
      events.add(new EventAndListeners(new AttributeChangeEvent(
          ...
          newValue, null), getListeners(
              notifier, propName)));
    } else { // eventType == Notification.REMOVE
    ...
    events.addAll(generateOppositeRefEvents(
        oldValue, newValue, eventType, oppositeRef));

    return events;
  }
  
  private List<EventAndListeners> generateOppositeRefEvents(
      Object oldValue,
      Object newValue,
      int eventType,
      EReference oppositeRef) {
    ...
    if (oppositeRef != null) {
      String propName = mapPropertyName(oppositeRef.getName());
      if (eventType == Notification.ADD) {
        events.add(new EventAndListeners(
  ...
  private void fireEventAndListeners(List<EventAndListeners> events) {
  ...
  private class EventAndListeners {

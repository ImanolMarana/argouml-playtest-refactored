...
class ExtensionMechanismsHelperEUMLImpl implements ExtensionMechanismsHelper {
    ...
    public void removeBaseClass(Object handle, Object baseClass) {
        ...
        removeBaseClass((Stereotype) handle, metaclass);
      }

      private void removeBaseClass(Stereotype st, Class metaclass) {
        Profile profile = st.getProfile();
        if (profile == null) {
            return;
        }
        for (Extension ext : profile.getOwnedExtensions(false)) {
            if (ext.getMetaclass() == metaclass
                ...
                removeExtension(st, ext);
                break;
            }
        }
      }

      private void removeExtension(Stereotype st, Extension ext) {

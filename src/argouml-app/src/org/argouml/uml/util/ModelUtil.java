...
public class ModelUtil {
    ...
    public static void generatePackageDependencies(Project project) throws IllegalModelElementConnectionException {
        ...
        for (Object model : models) {
            generateDependenciesForModel(model);
        }
    }

    private static void generateDependenciesForModel(Object model) throws IllegalModelElementConnectionException {
        for (Object classifier : Model.getModelManagementHelper().getAllModelElementsOfKindWithModel(model, Model.getMetaTypes().getClassifier())) {
            generateDependenciesForClassifier(classifier);
        }
    }

    private static void generateDependenciesForClassifier(Object classifier) throws IllegalModelElementConnectionException {
        Object namespace = Model.getFacade().getNamespace(classifier);
        if (Model.getFacade().getNamespace(namespace) != null) {
            Set dependentNamespaces = gatherDependentNamespaces(classifier);
            createDependencies(namespace, dependentNamespaces);
        }
    }

    private static Set gatherDependentNamespaces(Object classifier) {
    ...
    private static void createDependencies(Object namespace, Set dependentNamespaces) throws IllegalModelElementConnectionException {
